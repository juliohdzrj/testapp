...
if(isset($_SESSION["menuRedMedical"])){
	echo json_encode($_SESSION["menuRedMedical"]);
	exit(0);
}
...
$getMenu->tipoRed=4;//4:FICHA RED MEDICA
...
while($row=$resultGetMenu["code"]->fetch(PDO::FETCH_ASSOC)){
	...
	$category[]=array("url_name"=>$urlName,"name_category"=>utf8_encode($row["nombre_categoria"]),"id_category"=>$row["id_categoria"],"subcategory"=>"consulta-medica");
}
...
foreach($category as $key=>$data1){
	...
	foreach ($subCategory as $data2){
		if($data1["id_category"]==$data2){
			$category[$key]["subcategory"]="especialidades-medicas";
		}
	}
}
$dataResult=array("msg"=>$resultGetMenuSubCategory["msg"],"datosapi"=>$category);
$_SESSION["menuRedMedical"]=$dataResult;
echo json_encode($dataResult);
exit(0);